  ...
  const generateAndDownloadPDF = (data: any) => {
    ...
    cvElement.innerHTML = `
      <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; padding: 20px; max-width: 800px; margin: 0 auto;">
        <!-- Header with Photo -->
        <div style="display: flex; align-items: center; margin-bottom: 30px; border-bottom: 3px solid #10B981; padding-bottom: 20px;">
          <div style="margin-right: 25px; flex-shrink: 0;">
            <img src="${data.personalInfo.profilePhoto}" alt="${data.personalInfo.name}" 
                 style="width: 120px; height: 120px; border-radius: 12px; object-fit: cover; border: 3px solid #10B981;">
          </div>
          <div style="flex: 1;">
            <h1 style="font-size: 32px; font-weight: bold; color: #1f2937; margin-bottom: 8px;">${data.personalInfo.name}</h1>
            <h2 style="font-size: 18px; color: #10B981; margin-bottom: 15px;">${data.personalInfo.title}</h2>
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px; font-size: 14px; color: #4b5563;">
        ...
        <div style="background: linear-gradient(135deg, #f0fdf4, #ecfdf5); padding: 20px; border-radius: 10px; margin-bottom: 25px;">
          <p style="color: #065f46; font-size: 15px; line-height: 1.6;">
        ...
        <div style="margin-bottom: 25px;">
          <h3 style="font-size: 20px; font-weight: bold; color: #1f2937; margin-bottom: 15px; border-bottom: 2px solid #10B981; padding-bottom: 8px;">
            🎓 Education
          </h3>
          ${data.education.map((edu: any) => `
            <div style="margin-bottom: 18px; border-left: 3px solid ${edu.status === 'current' ? '#10B981' : '#e5e7eb'}; padding-left: 15px;">
              <div style="display: flex; justify-content: between; margin-bottom: 6px;">
                <div>
                  <h4 style="font-weight: bold; color: #1f2937; font-size: 16px; margin-bottom: 2px;">${edu.title}</h4>
                  <p style="color: #10B981; font-weight: 600; margin-bottom: 2px;">${edu.institution}</p>
                  <p style="color: #6b7280; font-size: 13px; margin-bottom: 8px;">${edu.location} • ${edu.period}</p>
                </div>
              </div>
              <p style="color: #4b5563; margin-bottom: 10px; line-height: 1.5;">${edu.description}</p>
              ${edu.grade ? `<span style="background: #f0fdf4; color: #059669; padding: 4px 12px; border-radius: 6px; font-size: 13px;">Grade: ${edu.grade}</span>` : ''}
            </div>
          `).join('')}
        </div>
        ...
        <div style="margin-bottom: 25px;">
          ...
          ${data.experience.map((exp: any) => `
            <div style="margin-bottom: 18px; border-left: 3px solid ${exp.status === 'current' ? '#10B981' : '#e5e7eb'}; padding-left: 15px;">
              <h4 style="font-weight: bold; color: #1f2937; font-size: 16px; margin-bottom: 2px;">${exp.title}</h4>
              <p style="color: #10B981; font-weight: 600; margin-bottom: 2px;">${exp.company}</p>
              <p style="color: #6b7280; font-size: 13px; margin-bottom: 8px;">${exp.location} • ${exp.period}</p>
              <p style="color: #4b5563; margin-bottom: 10px; line-height: 1.5;">${exp.description}</p>
              <div style="margin-top: 10px;">
                ${exp.achievements.map((achievement: string) => `
                  <p style="color: #4b5563; font-size: 13px; margin-bottom: 4px;">▶ ${achievement}</p>
                `).join('')}
              </div>
              <div style="margin-top: 10px;">
                ${exp.technologies.map((tech: string) => `
                  <span style="display: inline-block; background: #10B981; color: white; padding: 3px 10px; border-radius: 15px; font-size: 11px; margin-right: 6px; margin-bottom: 4px;">${tech}</span>
                `).join('')}
              </div>
            </div>
          `).join('')}
          ...
          <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
            ${data.skills.map((skillGroup: any) => `
              <div style="background: #f9fafb; padding: 15px; border-radius: 8px; border: 1px solid #e5e7eb;">
                <h4 style="font-weight: bold; margin-bottom: 12px; color: #1f2937;">${skillGroup.category}</h4>
                ${skillGroup.items.map((skill: any) => `
                  <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px; font-size: 13px;">
                    <span>${skill.icon} ${skill.name}</span>
                    <span style="font-weight: 600; color: #10B981;">${skill.level}%</span>
                  </div>
                `).join('')}
              </div>
            `).join('')}
        ...
        <div style="margin-bottom: 25px;">
          ...
          ${data.certifications.map((cert: any) => `
            <div style="margin-bottom: 18px; border-left: 3px solid #e5e7eb; padding-left: 15px;">
              <div style="display: flex; justify-content: space-between; margin-bottom: 6px;">
                <div>
                  <h4 style="font-weight: bold; color: #1f2937; font-size: 16px;">${cert.title}</h4>
                  <p style="color: #10B981; font-weight: 600;">${cert.issuer}</p>
                </div>
                <span style="color: #6b7280; font-size: 13px;">${cert.date}</span>
              </div>
              <p style="color: #4b5563;">${cert.description}</p>
            </div>
          `).join('')}
        </div>
        ...
        <div style="margin-bottom: 25px;">
          ...
          ${data.projects.map((project: any) => `
            <div style="background: #f9fafb; padding: 15px; border-radius: 8px; border-left: 4px solid #10B981; margin-bottom: 15px;">
              <h4 style="font-weight: bold; color: #1f2937;">${project.name}</h4>
              <p style="color: #4b5563; margin: 8px 0;">${project.description}</p>
              <div>
                ${project.tech.map((tech: string) => `
                  <span style="display: inline-block; background: #10B981; color: white; padding: 3px 10px; border-radius: 15px; font-size: 11px; margin-right: 6px;">${tech}</span>
                `).join('')}
              </div>
            </div>
          `).join('')}
    ...
    const options = {
      margin: 0.5,
      filename: 'FazilKhan_Malek_Professional_Resume.pdf',
    ...
    html2pdf()
      ...
      .save()
      .then(() => {
        console.log('✅ Professional CV PDF downloaded successfully!');
      })
      .catch((error: any) => {
